...
| `set-visibility` | `isolate` / `show` / `hide` / `reset`, with an animated fit-to-view so the human sees the agent's focus move. |
| `set-section` | One cut plane per axis, `offset` normalized 0..1 across the model's bounding box ("halfway up" needs no world coordinates). Returns the world coordinate the cut landed on. |
